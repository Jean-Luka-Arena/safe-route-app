...
from app.db.models import Ubicacion, Conexion, Incidente, Usuario
...
def sesion_con_datos(sesion_vacia):
    ...
    sesion_vacia.add(Usuario(id=1, email="test@test.com", password_hash="x"))
    sesion_vacia.commit()
    return sesion_vacia
...
def test_un_incidente_baja_la_seguridad_efectiva(sesion_con_datos):
    ...
    sesion_con_datos.add(
        Incidente(conexion_id=conexion.id, usuario_id=1, tipo="robo", gravedad=3)
    )
...
def test_varios_incidentes_se_acumulan(sesion_con_datos):
    ...
    sesion_con_datos.add_all(
        [
            Incidente(conexion_id=conexion.id, usuario_id=1, tipo="robo", gravedad=3),
            Incidente(
                conexion_id=conexion.id, usuario_id=1, tipo="zona_oscura", gravedad=2
            ),
        ]
    )
    sesion_con_datos.commit()
...
def test_la_seguridad_efectiva_no_baja_de_cero(sesion_con_datos):
    conexion = sesion_con_datos.query(Conexion).first()
    sesion_con_datos.add(
        Incidente(conexion_id=conexion.id, usuario_id=1, tipo="accidente", gravedad=100)
    )
    sesion_con_datos.commit()
...
def test_incidente_en_una_calle_no_afecta_a_otra(sesion_vacia):
    ...
    sesion_vacia.add(
        Incidente(conexion_id=conexion_ab.id, usuario_id=1, tipo="robo", gravedad=5)
    )
    sesion_vacia.commit()

    ciudad = obtener_ciudad(sesion_vacia)
    assert ciudad.obtener_atributos(1, 2).seguridad == 3
    assert ciudad.obtener_atributos(1, 3).seguridad == 8
